...
import { jumboify, parse, parseAllowLinks } from './markdown';


const MessageTimestamp = createClass({
  getDefaultProps() {
...
const DiscordViewWrapper = ({ children }) => {
  ...
  return (
    <div className='w-100 h-100 overflow-auto pa2 discord-view bg-[#36393e] rounded-md'>
...
const DiscordView = createClass({
  ...
  render() {
    ...
    const cls = `max-h-[calc(100vh-8rem)] h-full col-span-4 flex flex-col justify-between ${bgColor}`;
          ...
          <div className={`message-group hide-overflow ${compactMode ? 'compact' : ''}`}>
            ...
            <div className='comment'>
              <div className='message first'>
                ...
                {embed ? <Embed {...embed} /> : (embeds?.map((e, i) => <Embed key={`embed-${i}`} {...e} />))}
              </div>
