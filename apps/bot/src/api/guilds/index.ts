...
routes.openapi(
  {
    method: 'get',
    path: '/',
    summary: 'Retrieve all guilds the bot is a member of',
    tags: ['Guilds'],
    description:
      'Retrieves a list of guilds that the bot is a member of. Provide an API key to get guilds the bot is in.',
    security: [{ 'API Key': [] }],
    request: {
      query: z.object({
        page: z.number({ coerce: true }).min(1).optional().openapi({
          description: 'Page number to retrieve guilds from.',
          example: 1,
        }),
        limit: z.number({ coerce: true }).min(1).max(100).optional().openapi({
          description: 'Number of guilds to retrieve per page.',
          example: 10,
        }),
      }),
    },
          ...
          'application/json': {
            schema: z.object({
              ...
              totalPages: z.number().openapi({
                description: 'Total number of pages of guilds.',
                example: 1,
              }),
              guilds: z.array(
                guildSchema
  ...
  async (c) => {
    ...
    const guilds = client.guilds.cache.map((g) => g).slice((page - 1) * limit, page * limit)
      .map((g) => {
        ...
        return {
          ...
          hasBot: true,
          memberCount: g.members.cache.filter((m) => !m.user.bot).size,
        };
      }).sort((a, b) => {
...
routes.route('/', leaderboardRoutes);
